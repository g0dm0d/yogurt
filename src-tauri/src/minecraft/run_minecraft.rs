...
pub fn run(username: &str, uuid: &str, token: &str, instance: &str) {
    ...
    minecraft.arg("-Djava.library.path=".to_owned() + get_path(&format!("versions/{}/natives", config.version)).to_str().unwrap())
        ...
        .arg("release");
    println!("{:?}", minecraft);
    let output = minecraft.output().expect("Failed to start Minecraft client");
    println!("Minecraft client exited with status: {}", output.status);
    println!("Minecraft client stdout: {:?}", String::from_utf8_lossy(&output.stdout));
    println!("Minecraft client stderr: {:?}", String::from_utf8_lossy(&output.stderr));
}
